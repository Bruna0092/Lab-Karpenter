...
resource "aws_iam_role_policy_attachment" "policy_attach_AmazonSSMManagedInstanceCore" {
  policy_arn = "arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore"
  role       = aws_iam_role.nodes.name
}

########################################################################################################
resource "aws_eks_node_group" "node-group-management" {
  cluster_name    = aws_eks_cluster.cluster.name
  version         = var.cluster_version
  node_group_name = "node-group-management"
  node_role_arn   = aws_iam_role.nodes.arn

  subnet_ids = [
    var.private_subnets["eks-public-us-east-1a"],
    var.private_subnets["eks-public-us-east-1b"]
  ]
